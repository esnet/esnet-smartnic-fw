...
#define SERVER_CERT_COPY "/root/smartnic-p4-server.pem"

//--------------------------------------------------------------------------------------------------
string SmartnicP4Client::Options::get_address() const {
    if (!address.empty()) {
        return string(address);
    }

    const char* env = getenv("SN_P4_CLI_ADDRESS");
    if (env == NULL) {
        env = DEFAULT_ADDRESS;
    }
    return string(env);
...
//--------------------------------------------------------------------------------------------------
uint16_t SmartnicP4Client::Options::get_port() const {
    if (port != 0) {
        return port;
    }

    const char* env = getenv("SN_P4_CLI_PORT");
    if (env == NULL) {
        return DEFAULT_PORT;
    }
...
//--------------------------------------------------------------------------------------------------
string SmartnicP4Client::Options::get_auth_token() const {
    if (!auth_token.empty()) {
        return string(auth_token);
    }

    const char* env = getenv("SN_P4_CLI_AUTH_TOKEN");
    if (env == NULL) {
        env = "";
    }
    return string(env);
...
//--------------------------------------------------------------------------------------------------
string SmartnicP4Client::Options::get_tls_root_certs() const {
    if (!tls_root_certs.empty()) {
        return string(tls_root_certs);
    }

    const char* env = getenv("SN_P4_CLI_TLS_ROOT_CERTS");
    if (env == NULL) {
        env = "";
    }
    return string(env);
...
//--------------------------------------------------------------------------------------------------
string SmartnicP4Client::Options::get_tls_hostname_override() const {
    if (!tls_hostname_override.empty()) {
        return string(tls_hostname_override);
    }

    const char* env = getenv("SN_P4_CLI_TLS_HOSTNAME_OVERRIDE");
    if (env == NULL) {
        env = "";
    }
    return string(env);
...
//--------------------------------------------------------------------------------------------------
bool SmartnicP4Client::Options::get_tls_insecure() const {
    if (tls_insecure) {
        return true;
    }

    const char* env = getenv("SN_P4_CLI_TLS_INSECURE");
    if (env == NULL) {
        return false;
    }
...
//--------------------------------------------------------------------------------------------------
SmartnicP4Client::Stub SmartnicP4Client::connect(const SmartnicP4Client::Options& opts) {
    ...
    return SmartnicP4::NewStub(channel);
}
