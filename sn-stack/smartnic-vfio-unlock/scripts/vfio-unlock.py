...
def main(select, bar):
    ...
    # Dump out all available info for each device
    for device_addr, iommu_group_number in device_addr_to_iommu_group_number.items():
        print("Querying Device Info for", device_addr, "in group", iommu_group_number)

        # get the related group fd for this device
        group_fd = vfio_group_number_to_group_fd[iommu_group_number]

        # Open a DEVICE fd
        device_fd = fcntl.ioctl(group_fd, vfio.VFIO_GROUP_GET_DEVICE_FD, ctypes.create_string_buffer(bytes(device_addr,'utf-8')))
